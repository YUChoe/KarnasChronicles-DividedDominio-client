...
- [ ] 2. Gateway 서버 변경 (http.createServer + noServer 모드)
  - [ ] 2.1 gateway.ts를 `http.createServer` 기반으로 리팩토링
...
- [ ] 4. 체크포인트 - 서버 통합 및 인증 검증
  - 모든 테스트 통과 확인, 질문이 있으면 사용자에게 문의.

- [ ] 5. 데이터베이스 접근 계층 구현 (db-client.ts)
  - [ ] 5.1 DBClient 클래스 기본 구조 구현
...
- [ ] 6. 체크포인트 - 데이터베이스 계층 검증
  - 모든 테스트 통과 확인, 질문이 있으면 사용자에게 문의.

- [ ] 7. Admin Router 및 API 핸들러 구현
  - [ ] 7.1 AdminRouter 클래스 구현 (admin-router.ts)
  ...
  - [ ] 7.6 몬스터 API 핸들러 구현 (monsters-api.ts)
  ...
  - [ ] 7.7 게임 오브젝트 API 핸들러 구현 (objects-api.ts)
...
- [ ] 9. 프론트엔드 SPA 구현
  - [ ] 9.1 SPA 기본 구조 구현 (index.html, style.css)
  ...
  - [ ] 9.2 인증 UI 및 API 통신 모듈 구현 (app.js 일부)
  ...
  - [ ] 9.3 대시보드 및 월드 맵 렌더러 구현 (app.js 일부)
  ...
  - [ ] 9.7 게임 오브젝트 관리 UI 구현 (app.js 일부)
