...
`max_stack`은 서버가 보내지 않는다. 스택 병합이 화폐에만 구현되어 있어 이 값으로 판단할 수 있는 것이 없다. 수량은 `stack_count`로 표현되며 현재 화폐만 1을 초과한다.

같은 종류 아이템이 여럿일 때는 서버가 개별 엔티티로 보낸다. 체력 물약 4개는 uuid가 다른 엔티티 4개다. 클라이언트가 같은 `template_id`끼리 묶어 `×4`로 표시할 수 있으나, 액션을 보낼 때는 묶음 중 하나의 uuid를 지정한다.
