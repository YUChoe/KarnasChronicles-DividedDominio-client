...
- [x] 5.1 정적 페이지 작성
  - `src/server/public/`에 `index.html`, `style.css`, `app.js`를 작성한다. 빌드 도구를 사용하지 않는 정적 파일로 구성한다. 게임 소개, 스크린샷, 클라이언트 다운로드 링크를 포함한다. 영국 영어를 사용한다.
  - 회원가입 폼은 뺐다(2026-08-16 철회). 계정 생성은 클라이언트가 한다.
...
- [x] 5.2 정적 서빙 라우터
  - `src/server/landing/static.ts`에 정적 파일 서빙을 구현한다. (파일 이름은 회원가입 철회 후 `router.ts` 에서 바뀌었다.) 경로 순회 방어를 포함한다. 개발 환경에서 게이트웨이가 서빙하고 프로덕션에서는 nginx가 담당하므로, 서빙 활성화를 환경변수로 제어한다.
  ...
  - _Requirements: 5.8_
- [-] 5.3 계정 생성 클라이언트 — 철회
  - 철회(2026-08-16). 랜딩에 계정 생성을 두지 않기로 했다. 계정 생성은 Godot 클라이언트가 게임 채널의 `register` 로 직접 한다. 게이트웨이는 WebSocket 을 TCP 로 옮기는 일만 맡는다. `landing/router.ts`·`account-client.ts`·`rate-limit.ts`·`validate.ts` 와 테스트 36건을 지웠고 정적 서빙만 `landing/static.ts` 로 남겼다. `LANDING_SERVICE_TOKEN`, `REGISTER_LIMIT`, `REGISTER_WINDOW_MS` 와 nginx 의 `/api/` 위치도 함께 없앴다. 서버에서는 서비스 주체 인증(`service_login`)이 사라졌다. 남용은 다음 페이즈의 2차 인증으로 다룬다.
